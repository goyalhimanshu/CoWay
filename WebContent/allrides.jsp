...
<%
		String dep_place=request.getParameter("dep_place");
		String des_place=request.getParameter("des_place");
		String date=request.getParameter("date");
		String loginemail="";
		String picurl="";
		Connection con=DBInfo.getConnection();
		String totalride="select count(*) as total from rides where dep_place=? and des_place=? and date=? and approval=? ";
		PreparedStatement ps0=con.prepareStatement(totalride);
		ps0.setString(1, dep_place);
		ps0.setString(2, des_place);
		ps0.setString(3, date);
		ps0.setString(4, "true");
...
<br><br>
    <hgroup class="mb20">
		...
		<h2 class="lead"><strong class="text-danger"><%= total %></strong> results were found for the search for <strong class="text-danger"><%= dep_place%> <i class="glyphicon glyphicon-arrow-right"></i> <%=des_place%></strong></h2>								
	</hgroup>
	<section class="col-xs-12 col-sm-6 col-md-12">
	<%
		String selectloginemail="select * from userdata where userid=?";
		...
		String select="select * from rides where dep_place=? and des_place=? and approval=?";
		PreparedStatement ps=con.prepareStatement(select);
		ps.setString(1, dep_place);
		ps.setString(2, des_place);
		ps.setString(3, "true");
		...
		{		
			count++;
			PreparedStatement tempps=con.prepareStatement("select picurl from userdata where userid=?");
			tempps.setString(1, rs.getString("userid"));	//userid from ride table
    	...
    	<article class="search-result row">
			<div class="col-xs-12 col-sm-12 col-md-3">
				<a href="ShowRide.jsp?rideid=<%=rs.getString("rideid") %>" title="<%= dep_place%> -> <%=des_place%>" ><img class="thumbnail"src="<%=temprs.getString("picurl") %>" /></a>
			</div>
			<div class="col-xs-12 col-sm-12 col-md-3">
				<ul class="meta-search" >
					<li><i class="glyphicon glyphicon-calendar"></i> <span>Ride On: <%=rs.getString("date") %></span></li>
					<li><i class="glyphicon glyphicon-calendar"></i> <span>Ride Time: <%=rs.getString("time") %></span></li>
					<%-- <li><i class="glyphicon glyphicon-user"></i> <span> Posted by: <%=rs.getString("postedby") %></span></li> --%>
					<li><i class="glyphicon glyphicon-tags"></i> <span> INR <%=rs.getString("cost") %></span></li>
				</ul>
			</div>
			<div class="col-xs-12 col-sm-12 col-md-6 excerpet">
				<h3><a href="ShowRide.jsp?rideid=<%=rs.getString("rideid") %>" title="<%= dep_place%> -> <%=des_place%>"><%= dep_place%> <i class="glyphicon glyphicon-arrow-right"></i> <%=des_place%></a></h3>
				<p><%=rs.getString("description") %></p>
				<button type="submit" class="btn btn-default" id="contact<%= count%>">Contact </button>						
                <%
			      	String reportemail="";
			      	int flag=0;
			      	System.out.println("rideid"+rs.getString("rideid"));
			      	String selectreportemail="select * from report where reporteremail=? and rideid=?";
			      	PreparedStatement ps2=con.prepareStatement(selectreportemail);
			      	ps2.setString(1, loginemail);
			      	ps2.setString(2, rs.getString("rideid"));
